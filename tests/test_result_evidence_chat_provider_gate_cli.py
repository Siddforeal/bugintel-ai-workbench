...
def test_case_chat_provider_gate_cli_writes_markdown_and_json(tmp_path):
    ...
    assert "Case Chat Provider Gate" in result.output
...
def test_case_chat_provider_gate_cli_unsupported_provider(tmp_path):
    prompt_file = tmp_path / "prompt.json"
    prompt_file.write_text(json.dumps(_prompt_package()))
...
def test_case_chat_provider_gate_cli_missing_file_exits_nonzero(tmp_path):
    result = runner.invoke(app, ["case-chat-provider-gate", str(tmp_path / "missing.json")])

    assert result.exit_code == 1
    assert "Case chat prompt package JSON not found" in result.output


def test_case_chat_provider_gate_cli_invalid_json_exits_nonzero(tmp_path):
    prompt_file = tmp_path / "bad.json"
    prompt_file.write_text("{not json")

    result = runner.invoke(app, ["case-chat-provider-gate", str(prompt_file)])

    assert result.exit_code == 2
    assert "Invalid case chat prompt package JSON" in result.output


def test_case_chat_provider_gate_cli_wrong_kind_exits_nonzero(tmp_path):
    prompt_file = tmp_path / "prompt.json"
    ...
    assert result.exit_code == 2
    assert "Invalid case chat provider gate input" in result.output
